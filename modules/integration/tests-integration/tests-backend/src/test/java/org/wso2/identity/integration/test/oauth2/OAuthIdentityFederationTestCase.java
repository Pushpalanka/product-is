...
public class OAuthIdentityFederationTestCase extends AbstractIdentityFederationTestCase {

    private static final String PRIMARY_IS_SERVICE_PROVIDER_NAME = "playground2";
    ...
    private static final String PRIMARY_IS_OAUTH_APPLICATION_NAME = "playground2";
    private static final String PRIMARY_IS_OAUTH2_CALLBACK_URL = "http://localhost:8490/playground2/oauth2client";
    private static final String SECONDARY_IS_OAUTH_APPLICATION_NAME = "OAuthFedSP";
    ...
    private static final String INBOUND_AUTH_TYPE = "oauth2";//ToDO
    private static final int TOMCAT_8490 = 8490;
    private static final int PORT_OFFSET_0 = 0;
    private static final int PORT_OFFSET_1 = 1;
    private static final String OIDCAUTHENTICATOR = "OIDCAuthenticator"; //ToDO
...
//                                                                                        File.separator + "org.wso2.sample.is.sso.agent.war").getPath());
    ...
    public void endTest() throws Exception {
        ...
        super.deleteSAML2WebSSOConfiguration(PORT_OFFSET_0, PRIMARY_IS_OAUTH_APPLICATION_NAME);
        ...
        super.deleteSAML2WebSSOConfiguration(PORT_OFFSET_1, SECONDARY_IS_OAUTH_APPLICATION_NAME);
    ...
    public void testCreateIdentityProviderInPrimaryIS() throws Exception {
        ...
        saml2SSOAuthnConfig.setDisplayName("oauth2");
    ...
    public void testCreateServiceProviderInPrimaryIS() throws Exception {
        ...
        updateServiceProviderWithSAMLConfigs(PORT_OFFSET_0, PRIMARY_IS_OAUTH_APPLICATION_NAME,
                PRIMARY_IS_OAUTH2_CALLBACK_URL, serviceProvider);
        ...
        if (configs != null) {
            for (InboundAuthenticationRequestConfig config : configs) {
                if (PRIMARY_IS_OAUTH_APPLICATION_NAME.equals(config.getInboundAuthKey()) && INBOUND_AUTH_TYPE.equals(config.getInboundAuthType())) {
                    success = true;
                    break;
                }
            }
    ...
    public void testCreateServiceProviderInSecondaryIS() throws Exception {
        ...
        updateServiceProviderWithSAMLConfigs(PORT_OFFSET_1,
                SECONDARY_IS_OAUTH_APPLICATION_NAME, String.format(COMMON_AUTH_URL, DEFAULT_PORT + PORT_OFFSET_0), serviceProvider);
        ...
        if (configs != null) {
            for (InboundAuthenticationRequestConfig config : configs) {
                if (SECONDARY_IS_OAUTH_APPLICATION_NAME.equals(config.getInboundAuthKey()) && INBOUND_AUTH_TYPE.equals(config.getInboundAuthType())) {
                    success = true;
                    break;
                }
            }
    ...
    public void testAttributeConsumerIndex() throws Exception {
        ...
        for (int i = 0; i < federatedAuthenticatorConfigs.length; i++) {
            if (OIDCAUTHENTICATOR.equals(federatedAuthenticatorConfigs[i].getName())) {
    ...
    private String authenticateWithSecondaryIS(HttpClient client, String sessionId)
        ...
        request.addHeader("User-Agent", USER_AGENT);
        request.addHeader("Referer", PRIMARY_IS_OAUTH2_CALLBACK_URL);
    ...
    private boolean sendSAMLResponseToWebApp(HttpClient client, String samlResponse)
        ...
        HttpPost request = new HttpPost(PRIMARY_IS_OAUTH2_CALLBACK_URL);
        request.setHeader("User-Agent", USER_AGENT);
        List<NameValuePair> urlParameters = new ArrayList<NameValuePair>();
